...
Run at the end of Feature, Bug fix, Perf issue, Refactoring, Visual parity, UI design, Hillclimb, and Authoring a skill. Not Investigation, Prototype, Eval, Pause, Session pickup, Multi-phase, Runtime or Trace forensics, Worktree cleanup, Autonomous run, Babysit, Shipping, Orchestrate, or either Autopilot.
...
**PRs.** Run `/deslop` from `cursor-team-kit` over the diff before commit. If `/deslop` is missing, ask for the same outcome in plain words. Run `/no-comments` before review. Write every PR title, PR description, and commit body with `/technical-writing`, then apply `/unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `utopia-stack` or `utopia-mode`, as the scope. Keep the subject short and imperative. Apply the same `/technical-writing` and `/unslop` pass as the body. Name a real symbol when one carries the change. For example, `fix(utopia-stack): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
**Size and stacks.** Prefer five narrow PRs to one large PR. Stack follow-ups with Graphite (`gt`), and keep the ordered stack visible to reviewers. If `gt` is missing, say so. Use `gh` for PRs and rebase onto main. Do not assume `gt submit` or stack commands. Branch from main only for independent work. Rebase on `main` before substantial stack work.
