...
const PROGRAM_MARKERS = ["/goal", "skill-root", /30[- ]minute/, "status message"];
...
for (let i = start; i < raw.length; i++) {
	...
	if (/: \S/.test(prose.replace(/\bn\/a: \S+/g, ""))) fail(n, "mid-sentence colon");
}
...
for (const pr of prSections) {
	...
	const afterRule = (rest) => (rest.startsWith(RULE) ? rest.slice(RULE.length) : rest);
	const markedNa = (rest) => afterRule(rest).includes("n/a:");

	const live = block("Verify, live.");
	if (live) {
		const liveBoxes = boxes(live.lines);
		if (markedNa(live.rest)) {
			if (liveBoxes.length) fail(live.n, `${pr.title}: Verify, live is n/a but has lane boxes`);
		} else {
			if (!live.rest.includes(LANES)) fail(live.n, `${pr.title}: Verify, live lacks "${LANES}"`);
			const lanes = liveBoxes.map((b) => ({ ...b, m: b.text.match(/^Lane (\d+)\. /) }));
	...
	if (perf) {
		...
		if (markedNa(perf.rest)) {
			if (perfBoxes.length) fail(perf.n, `${pr.title}: Verify, perf is n/a but has boxes`);
		} else {
