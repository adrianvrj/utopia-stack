...
```bash
node <skill-root>/scripts/live.mjs
```
...
```
LOOP:
  node <skill-root>/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```
node <skill-root>/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node <skill-root>/scripts/live-status.mjs
node <skill-root>/scripts/live-resume.mjs --id SESSION_ID
node <skill-root>/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node <skill-root>/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node <skill-root>/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node <skill-root>/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node <skill-root>/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node <skill-root>/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node <skill-root>/scripts/live-server.mjs stop
```
...
```bash
node <skill-root>/scripts/detect-csp.mjs
```
