...
const HOOK_MANIFEST_TARGETS = [
  ...
  // Hooks only work if this skill is copied into the target repo. The plugin
  // cache path is not $(git rev-parse --show-toplevel)/skills/impeccable.
  {
    provider: '.agents',
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: 'command',
                command: 'node "$(git rev-parse --show-toplevel)/skills/impeccable/scripts/hook.mjs"',
                timeout: TIMEOUT_SECONDS,
                statusMessage: STATUS_MESSAGE,
              },
            ],
