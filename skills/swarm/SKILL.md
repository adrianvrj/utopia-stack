...
4. Pick the worker model from `swarm workers` in `~/.cursor/rules/utopia-stack-models.mdc` when present. Otherwise use `grok-4.6-fast-xhigh`. For a model race, name each arm's model up front.
