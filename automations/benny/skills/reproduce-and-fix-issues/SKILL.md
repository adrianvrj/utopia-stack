...
- Use utopia-stack's `principle-guard-the-context-window` for delegated analysis.
- Apply utopia-stack's `principle-sequence-verifiable-units`, `principle-fix-root-causes`, and `principle-prove-it-works` through repro, fix, and verification.
...
Use utopia-stack's `how` skill to trace the action through the repository. Use `why` for regression history and defensive code. Form competing cause hypotheses and identify evidence that would separate them.
...
- Invoke utopia-stack's `tdd` skill when there is a cheap local test target, and write the failing test before the fix.
...
- Run the pull request text and all Slack updates through utopia-stack's `unslop` skill.
