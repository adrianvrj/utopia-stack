...
- Apply utopia-stack's `principle-separate-before-serializing-shared-state` to source coordinates.
- Apply utopia-stack's `principle-minimize-reader-load` and `unslop` skills to the final verdict.
...
Do a bounded source and history pass before choosing an owner or destination. Use utopia-stack's `how` skill to trace the path from the reported action to the observed result. Use `why` when the report looks like a regression or touches defensive code.
