...
![Four prompt cards: /how, /tdd, /loop, /unslop.](./images/recipes.jpg)
...
```text
/utopia-mode repro the duplicate write first. if there's a cheap test path, /tdd it. then fix and rerun.
```
...
- **Writing a `SKILL.md` freehand.** Route it through the [Authoring or modifying a skill playbook](../../skills/utopia-mode/playbooks/authoring-a-skill.md) so validation and review happen.
