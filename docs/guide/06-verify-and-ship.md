...
![It compiles is crossed out; the real artifact box shows verify: pass and evidence: captured.](./images/verification.jpg)
...
```text
/utopia-mode add json output to this command. text output stays byte-identical, the json parses, both run against the sample project. show me the evidence.
```
...
```text
/utopia-mode open the pr. small ordered commits, evidence in the description.
```

The [Opening a PR playbook](../../skills/utopia-mode/playbooks/opening-a-pr.md) works from a worktree, rebases the work into small ordered commits, cleans the diff, unslops the prose, and returns the PR link. Five narrow PRs beat one fat one, and stacked follow-ups beat a growing branch.

## Drive the PR to merge-ready with Babysit

An open PR starts collecting blockers immediately. Checks fail, reviewers comment, trunk moves. Hand that churn to the [Babysit playbook](../../skills/utopia-mode/playbooks/babysit.md):

```text
/utopia-mode babysit this pr. get it green.
```
...
```text
/utopia-mode check on pr 123. anything outstanding?
```
...
```text
/utopia-mode land the stack.
```

The [Shipping playbook](../../skills/utopia-mode/playbooks/shipping.md) verifies each PR independently before it arms anything. One fresh agent per PR proves the behavior live, and the agent that judges a change is never the one that wrote it. Then Shipping lands only the contiguous verified run from the bottom, through Graphite merge-when-ready, and reports the first PR that breaks the chain. If `gt` is missing, PRs that already target `main` land one at a time with `gh`; a stack without Graphite stops. A verified PR sitting above an unverified one waits, because merging it would pull the gap in underneath.
