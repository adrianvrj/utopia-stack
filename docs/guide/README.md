# The utopia-stack guide

utopia-stack works best when you stop micromanaging the agent. You describe what you want and how you'll know it's done. `/utopia-mode` picks the playbook, runs the other skills as the steps need them, and shows you the evidence. This guide teaches that habit with realistic prompts.

Here's what you'll learn:

1. [Set up utopia-stack](./01-setup.md). Install the plugin and pick your models.
2. [Route work through `/utopia-mode`](./02-utopia-mode.md). Give it a goal and watch it pick a playbook.
...
```text
/utopia-mode the export writes duplicate rows when a retry lands mid-run. repro first, then fix and verify.
```

You don't need to name a playbook or list skills. "repro first" and a checkable outcome are all the routing signal `/utopia-mode` needs. It matches the Bug fix playbook, copies the steps into a todo list, and calls the right skills as each step fires.

Next: [Set up utopia-stack](./01-setup.md).
