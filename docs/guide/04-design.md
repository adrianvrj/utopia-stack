...
![Three cards for /architect, /arena, and /interrogate feeding one pick-and-ship bar.](./images/design.jpg)
...
The panel comes from your [`/setup-utopia-stack`](../../skills/setup-utopia-stack/SKILL.md) configuration, and you can adjust it per task. Ask for more candidates when the decision matters, fewer when it doesn't:
...
- A change that introduces or changes a type, module, or ownership boundary, or that you asked to architect, earns `/architect`, which brings `/arena` with it.
...
`/utopia-mode` already applies this ladder. A type, module, or ownership boundary, or you asking to architect, triggers `/architect` on its own, so you reach for these directly mainly when you want more or less scrutiny than the default.

## When the design is visual

Architect, arena, and interrogate settle system shape. When the work is the look — a landing, a redesign, polish, an image-first build — `/utopia-mode` matches the [UI design playbook](../../skills/utopia-mode/playbooks/ui-design.md) instead.

```text
/utopia-mode design this landing. premium, not templated. verify in the browser.
```

That playbook infers a Design Read, picks image-first or code-first, implements through one skill, polishes, then verifies in the browser. Feature still owns new behavior. Prototype still owns throwaway sketches. Visual parity still owns pixel-match.
